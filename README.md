...
Вони охоплюють `200`, порожній `200` з `[]`, `400` та `404`, а також три
сценарії реалізованої точки розширення `severity-summary`.
...
`GET /api/incidents/severity-summary`, який у baseline повертав `501`.

**Стан у цій гілці:** точку розширення реалізовано. Endpoint повертає `200` з
підсумком за рівнями критичності й підтримує необовʼязковий параметр `status`,
який перевіряється за allowlist (некоректне значення дає `400`). Політика
нульових груп — повний перелік рівнів enum: рівень без інцидентів має `count: 0`.
Порядок — за критичністю (`Low`, `Medium`, `High`, `Critical`), а не
лексикографічний, тому агрегат сортується в памʼяті після `ToListAsync`.
